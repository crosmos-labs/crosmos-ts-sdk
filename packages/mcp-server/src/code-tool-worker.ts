...
import { Crosmos, ClientOptions } from 'crosmos';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Crosmos)`
    : `const run: (${functionSource.client}: Crosmos) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
