...
import { ClientOptions } from 'crosmos';

const prompt = `Runs JavaScript code to interact with the Crosmos API.
...
async function run(client) {
  const search = await client.search.hybrid({ query: 'What is my primary language?', space_id: '<your-space-uuid>' });
...
const remoteStainlessHandler = async ({
  ...
  const localClientEnvs = {
    CROSMOS_API_KEY: requireValue(
      readEnv('CROSMOS_API_KEY') ?? client.apiKey,
      'set CROSMOS_API_KEY environment variable or provide apiKey client option',
    ),
    CROSMOS_BASE_URL: readEnv('CROSMOS_BASE_URL') ?? client.baseURL ?? undefined,
  };
