...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['CROSMOS_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['CROSMOS_LOG'] or 'warn' if it isn't set.
   */
...
export class Crosmos {
   ...
   */
  constructor({
    baseURL = readEnv('CROSMOS_BASE_URL'),
    apiKey = readEnv('CROSMOS_API_KEY'),
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.CrosmosError(
        "The CROSMOS_API_KEY environment variable is missing or empty; either provide it, or instantiate the Crosmos client with an apiKey option, like new Crosmos({ apiKey: 'My API Key' }).",
      );
    }
    ...
    this.timeout = options.timeout ?? Crosmos.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('CROSMOS_LOG'), "process.env['CROSMOS_LOG']", this) ??
      defaultLogLevel;
  ...
  static CrosmosError = Errors.CrosmosError;
...
export declare namespace Crosmos {
  ...
  export {
    ...
    type SearchHybridParams as SearchHybridParams,
  };
